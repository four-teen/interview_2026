 ...
 * Uses the same ranking logic and ordering used by Program Chair ranking.
 */
...
if (!$stmtProgram) {
    ...
    echo json_encode([
        'success' => false,
        'message' => 'Server error (program query).'
    ]);
    exit;
}
...
while ($row = $resultRanking->fetch_assoc()) {
    $mappedRow = [
        ...
        'sat_score' => (int) ($row['sat_score'] ?? 0),
        'final_score' => number_format((float) ($row['final_score'] ?? 0), 2)
    ];

    if ($mappedRow['interview_id'] <= 0) {
        continue;
    }
